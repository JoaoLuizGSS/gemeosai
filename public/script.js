document.addEventListener('DOMContentLoaded', () => {
    ...
    function login(username) {
        ...
        if (userBadge) userBadge.innerText = username.charAt(0);
        loadUserHistory();
        startNewChat();
    }
    ...
    function createImageMessageElement(src) {
        const div = document.createElement('div');
        div.classList.add('message', 'user');
    ...
    function createFileCardElement(name, size) {
        const div = document.createElement('div');
        div.classList.add('message', 'user');
        div.innerHTML = `<div class="uploaded-file-card"><i class="fa-solid fa-file-lines"></i><div class="file-info"><span class="file-name">${name}</span><span class="file-size">${size}</span></div></div>`;
        if (chatContainer) chatContainer.appendChild(div);
        scrollToBottom();
    }
